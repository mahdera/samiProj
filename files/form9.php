<?php
    ...
    function getForm9ModifiedByUserOnThisDate($modifiedBy, $modificationDate){
        try{
            $query = "select * from tbl_form_9 where modified_by = $modifiedBy order by modification_date desc limit 0,1";
            $result = read($query);
            $resultRow = mysql_fetch_object($result);
            return $resultRow;
        } catch (Exception $ex) {
    ...
    function getLatestForm9ModifiedByUsingUserLevelResultSet($userLevel, $divisionId){
      try{
        $query = null;
        if($userLevel == '02'){
          $query = "select tbl_form_9.* from tbl_form_9, tbl_user_sub_district where " .
          "tbl_form_9.modified_by = tbl_user_sub_district.user_id and " .
    ...
    function getAllForm9ModifiedByUsingUserLevel($userLevel, $divisionId){
      try{
        $query = null;
        if($userLevel == '02'){
          $query = "select tbl_form_9.* from tbl_form_9, tbl_user_sub_district where " .
    ...
    function getForm9ModifiedByUserUsingLevel($userLevel, $divisionId){
      try{
        $query = null;
        if($userLevel == '02'){
          $query = "select tbl_form_9.* from tbl_form_9, tbl_user_sub_district where tbl_form_9.modified_by = tbl_user_sub_district.user_id and " .
          "tbl_user_sub_district.sub_district_id = $divisionId order by tbl_form_9.modification_date desc limit 0,1";
          $result = read($query);
          $resultRow = mysql_fetch_object($result);
          return $resultRow;
    ...
    function checkIfForm9RecordIsAlreadyEntered($userLevel, $divisionId){
      try{
        $query = null;
        $cnt = 0;
        if($userLevel == '02'){
          $query = "select count(*) as cnt from tbl_form_9, tbl_user_sub_district where " .
          "tbl_form_9.modified_by = tbl_user_sub_district.user_id and " .
